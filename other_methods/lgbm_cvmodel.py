...
class ModelWrapper(object):
    ...
    def fit(self):
        ...
        total_ll = 0
        total_num = 0
        if (self.Y.dtypes == 'O' or self.Y.dtypes == 'bool' or 
            self.Y.dtype.name == 'category' or self.Y.dtypes == 'int'):
            ...
            residule = np.array([])
            if n_split == 0:
                presidule, sumll = self.pred_model.fit(X_train=self.X, y_train=self.Y,
                                                       X_test=self.X, y_test=self.Y)
